import { Recipe, RecipeImport, PreparationStep, CookingStep, ShoppingItem } from '../types/recipe';
...
export const parseRecipeJson = (jsonData: RecipeImport): Recipe => {
  ...
  const preparationSteps: PreparationStep[] = jsonData.preparationSteps.map((step, index) => {
  ...
  const cookingSteps: CookingStep[] = jsonData.cookingSteps.map((step, index) => {
    if (typeof step === 'string') {
      return {
        stepNumber: index + 1,
        instruction: step,
 ...
 */
export const validateRecipeJson = (data: any): { valid: boolean; error?: string } => {
  ...
  if (!Array.isArray(data.preparationSteps) && !Array.isArray(data.cookingSteps)) {
    return { valid: false, error: 'Recipe must have preparation steps or cooking steps' };
  }
